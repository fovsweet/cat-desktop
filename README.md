...
| 交互 | 效果 |
|------|------|
| 移动鼠标 | 宠物转头看向光标(抬头/低头/侧头)+ 身体轻微倾斜 |
| 鼠标离远 | 起身迈着步子走向鼠标(摇摆+颠簸步行动画),走近后停下注视 |
| 左键单击宠物 | 伸出毛色匹配的爪子拍向光标 |
| 快速连点 | 整只宠物扑过去捕捉鼠标,落地冒爱心 |
| 右键 → 投喂 | 小鱼干🐟 / 鸡腿🍗 / 牛奶🥛 落地,低头进食 |
| 右键 → 抚摸 | 开心扭动 + 爱心粒子 |
| 90 秒无操作 | 垂头打盹飘 Zzz,鼠标靠近或点击会醒 |
| 按住拖动 | 把宠物摆到屏幕任意位置 |
...
- **抠图**:`VNGenerateForegroundInstanceMaskRequest`,本地完成;多主体时自动取最大的
- **眼睛定位**:`VNDetectAnimalBodyPoseRequest`(支持猫/狗),失败时手动标记兜底
- **动画**:2.5D 木偶 —— 抠图作身体层,以双眼为中心羽化裁出头部图层,
  绕颈部支点转头;步行循环、爪子、粒子均为程序化 CALayer 动画
- **窗口移动**:定时器逐帧位移(走动/扑捉),不用 `window.animator()` 以避免快照叠影
- **鼠标跟踪**:轮询 `NSEvent.mouseLocation`,无需辅助功能权限
- **窗口**:透明无边框 `NSPanel`,置顶、全空间可见、不抢焦点;单实例守卫防止重叠
